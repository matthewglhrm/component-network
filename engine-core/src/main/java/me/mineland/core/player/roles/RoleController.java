...
public enum RoleController {
    ...
    public boolean hasRolePermission(ProxiedPlayer proxiedPlayer) {
    ...
    public static RoleController getRolePlayer(ProxiedPlayer proxiedPlayer) {
        for (RoleController role : values()) {
            if (role.hasRolePermission(proxiedPlayer)) {
                return role;
            }
        }
        return MEMBRO;
